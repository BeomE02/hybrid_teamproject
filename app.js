...
async function triggerEmergency(mode) {
    ...
    if (mode === 'flash') {
        ...
        // 3. SOS (화면 검정 + 플래시 패턴)
        overlay.style.backgroundColor = 'black'; // 화면은 검게
        isSOSOn = true;
        runSOSPattern();
    }
}
...
function emergencyStop() {
    ...
    // 화면 플래시도 끄기
    const screenFlash = document.getElementById('screenFlashOverlay');
...
async function setHardwareFlash(on) {
    ...
    // 화면 플래시는 SOS 모드가 아닐 때(손전등/사이렌)만 작동
    if (emergencyMode !== 'sos') {
